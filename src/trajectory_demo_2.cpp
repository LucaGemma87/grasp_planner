...
private:
 ...
 //! Subscribes to and advertises topics; initializes fitter and marker publication flags
  ...
  TrajectoryDemos2(ros::NodeHandle nh) : nh_(nh), priv_nh_("~")
  {
   ...
   grasp_planning_srv_ = nh_.advertiseService(nh_.resolveName("grasp_planning_srv"),    &TrajectoryDemos2::serviceCallback, this);
 ...
 */
bool TrajectoryDemos2::serviceCallback(TrajectoryDemo::Request &request, TrajectoryDemo::Response &response)
{ 
  int n_object;
  n_object=request.ob;
  double pi(3.1415);
  ...
  if(n_object==(int)1)
  {
   ...
   // Set the scale of the marker -- 1x1x1 here means 1m on a side
    ...
    cyl_marker.lifetime = ros::Duration(30);
    ...
    { 
      ...
      if ((position_cylinder[0] != position_wrist_home[0]) & (position_cylinder[1] != position_wrist_home[1]))
      {
       ...
       if(position_cylinder[2] !=position_wrist_home[2])
       {
         ...
         double sigma4=pow(position_cylinder[0],2)-2*position_cylinder[0]*position_wrist_home[0]-2*position_cylinder[0]*position_cylinder[1]+2*position_cylinder[0]*position_wrist_home[1]+pow(position_wrist_home[0],2)+2*position_wrist_home[0]*position_cylinder[1]-2*position_wrist_home[0]*position_wrist_home[1]+pow(position_cylinder[1],2)-2*position_cylinder[1]*position_wrist_home[1]+pow(position_wrist_home[1],2)+pow(position_cylinder[2],2)-(double)2*position_cylinder[2]*position_wrist_home[2]+pow(position_wrist_home[2],2);
         ROS_INFO("sigma4 = %f",sigma4);

         double sigma5=pow(position_cylinder[0],2)-2*position_cylinder[0]*position_wrist_home[0]+2*position_cylinder[0]*position_cylinder[1]-2*position_cylinder[0]*position_wrist_home[1]+pow(position_wrist_home[0],2)-2*position_wrist_home[0]*position_cylinder[1]+2*position_wrist_home[0]*position_wrist_home[1]+pow(position_cylinder[1],2)-2*position_cylinder[1]*position_wrist_home[1]+pow(position_wrist_home[1],2)+pow(position_cylinder[2],2)-(double)2*position_cylinder[2]*position_wrist_home[2]+pow(position_wrist_home[2],2);
         ROS_INFO("sigma20 = %f",sigma5);

         double sigma3=sqrt(sigma4*sigma5);
         ROS_INFO("sigma3 = %f",sigma3);

         double sigma1=(sigma3-2*position_cylinder[0]*position_wrist_home[0]-2*position_cylinder[1]*position_wrist_home[1]+2*position_cylinder[2]*position_wrist_home[2]-pow(position_cylinder[0],2)+pow(position_wrist_home[0],2)+pow(position_cylinder[1],2)+pow(position_wrist_home[1],2)-pow(position_cylinder[2],2)-pow(position_wrist_home[2],2))/((double)2*(position_cylinder[2]-position_wrist_home[2]));
         ...
         //Roll_wrist_end=(double)();
         Roll_wrist_end=(double)(0);
         Pitch_wrist_end=(double)(-pi/2);
         //Pitch_wrist_end=(double)(-pi/2);
         //Yaw_wrist_end=(double)(-pi/2+2*theta);
         Yaw_wrist_end=(double)(2*theta);
         ...
         for (int i = 0; i < int_traj; ++i)
         {
           ...
           { 
             ...
             z_wrist_traj[i]=pow(x_wrist_traj[i]-position_cylinder[0],2)/alpha2+pow(y_wrist_traj[i]-position_cylinder[1],2)/beta2+position_cylinder[2];
            }
 ...
 for (int i = 0; i < int_traj; ++i)//int_traj
  ...
  double inc_treshold((double)0.05);
  double inc_eef_step((double)0.001);
  int int_jump_max(10);
  ...
  {  
   for (int i = 0; i < int_jump_max; ++i)
   {
     ...
     group.setGoalOrientationTolerance((double)pi/(2*90)); 
     ...
     if((double)fraction>(double)0.90)
     {
       ...
       // name_file_ptr.open("/home/luca/file_txt/cylinder_trajectory_demo_2.txt",std::ios::app);
...
int main(int argc, char **argv) 
{
  ...
  grasp_planner::TrajectoryDemos2 node(nh);
