...
private:
 ...
 //! Subscribes to and advertises topics; initializes fitter and marker publication flags
  ...
  TrajectoryDemos3(ros::NodeHandle nh) : nh_(nh), priv_nh_("~")
  {
   ...
   grasp_planning_srv_ = nh_.advertiseService(nh_.resolveName("grasp_planning_srv"),    &TrajectoryDemos3::serviceCallback, this);
 ...
 */
bool TrajectoryDemos3::serviceCallback(TrajectoryDemo::Request &request, TrajectoryDemo::Response &response)
{ 
  int n_object;
  n_object=request.ob;
  double pi(3.1415);
  ...
  if(n_object==(int)1)
  {
   ...
   // Set the scale of the marker -- 1x1x1 here means 1m on a side
    ...
    { 
      ...
      int int_traj =(int)3;
      double int_step = (double)3;
      
      while(int_traj<(int)99)
      {
      ...
      {
       double alpha1=-(pow(position_cylinder[1]-position_wrist_home[1],2))/(position_cylinder[0]-position_wrist_home[0]); 
       ...
       if(position_cylinder[2] !=position_wrist_home[2])
       {
         ...
         double sigma4=(double)20*pow(position_cylinder[1]*position_wrist_home[1],3);
          ROS_INFO("sigma4 = %f",sigma4);


         double sigma5=(double)15*pow(position_cylinder[1],4)*pow(position_wrist_home[1],2);
         ROS_INFO("sigma5 = %f",sigma5);

         
         double sigma6=(double)15*pow(position_cylinder[1],2)*pow(position_wrist_home[1],4);
         ROS_INFO("sigma6 = %f",sigma6);

         double sigma7=(double)20*pow(position_cylinder[0],3)*pow(position_wrist_home[0],3);
         ROS_INFO("sigma7 = %f",sigma7);

         double sigma8=(double)15*pow(position_cylinder[0],4)*pow(position_wrist_home[0],2);
         ROS_INFO("sigma8 = %f",sigma8);
         
         double sigma9=(double)15*pow(position_cylinder[0],2)*pow(position_wrist_home[0],4);
         ROS_INFO("sigma9 = %f",sigma9);

         double sigma10=(double)6*pow(position_wrist_home[0],5)*position_wrist_home[1];
         ROS_INFO("sigma10 = %f",sigma10);
         
         double sigma11=(double)6*position_cylinder[1]*pow(position_wrist_home[1],5);
         ROS_INFO("sigma11 = %f",sigma11);

         double sigma12=(double)6*pow(position_cylinder[0],5)*position_wrist_home[0];
         ROS_INFO("sigma12 = %f",sigma12);

         double sigma13=(double)6*position_cylinder[0]*pow(position_cylinder[0],5);
         ROS_INFO("sigma13 = %f",sigma13);
         
         double sigma14=(double)18*pow(position_cylinder[0],2)*position_wrist_home[0]*pow(position_cylinder[1],2)*position_wrist_home[1];
         ROS_INFO("sigma14 = %f",sigma14);

         double sigma15=(double)18*pow(position_cylinder[0],2)*position_wrist_home[0]*position_cylinder[1]*pow(position_wrist_home[1],2);
         ROS_INFO("sigma15 = %f",sigma15);

         double sigma16=(double)18*position_cylinder[0]*pow(position_wrist_home[0],2)*pow(position_cylinder[1],2)*position_wrist_home[1];
         ROS_INFO("sigma16 = %f",sigma16);

         double sigma17=(double)18*position_cylinder[0]*pow(position_wrist_home[0],2)*position_cylinder[1]*pow(position_wrist_home[1],2);
         ROS_INFO("sigma17 = %f",sigma17);

         double sigma18=(double)6*pow(position_wrist_home[0],3)*pow(position_cylinder[1],2)*position_wrist_home[1];
         ROS_INFO("sigma18 = %f",sigma18);

         double sigma19=(double)6*pow(position_wrist_home[0],3)*position_cylinder[1]*pow(position_wrist_home[1],2);
         ROS_INFO("sigma19 = %f",sigma19);

         double sigma20=(double)6*pow(position_cylinder[0],3)*pow(position_cylinder[1],2)*position_wrist_home[1];
         ROS_INFO("sigma20 = %f",sigma20);

         double sigma21=(double)6*pow(position_cylinder[0],3)*position_cylinder[1]*pow(position_wrist_home[1],2);
         ROS_INFO("sigma21 = %f",sigma21);

         double sigma22=(double)6*pow(position_cylinder[0],2)*position_wrist_home[0]*pow(position_wrist_home[1],3);
         ROS_INFO("sigma22 = %f",sigma22);

         double sigma23=(double)6*position_cylinder[0]*pow(position_wrist_home[0],2)*pow(position_wrist_home[1],2);
         ROS_INFO("sigma23 = %f",sigma23);

         double sigma24=(double)6*pow(position_cylinder[0],2)*position_wrist_home[0]*pow(position_cylinder[1],3);
         ROS_INFO("sigma24 = %f",sigma24);

         double sigma25=(double)6*position_cylinder[0]*pow(position_wrist_home[0],2)*pow(position_cylinder[1],3);
         ROS_INFO("sigma25 = %f",sigma25);

         double sigma26=(double)2*pow(position_wrist_home[0],3)*pow(position_wrist_home[1],3);
         ROS_INFO("sigma26 = %f",sigma26);

         double sigma27=(double)2*pow(position_wrist_home[0],3)*pow(position_cylinder[1],3);
         ROS_INFO("sigma27 = %f",sigma27);

         double sigma28=(double)2*pow(position_cylinder[0],3)*pow(position_wrist_home[1],3);
         ROS_INFO("sigma28 = %f",sigma28);

         double sigma29=(double)2*pow(position_cylinder[0],3)*pow(position_cylinder[1],3);
         ROS_INFO("sigma29 = %f",sigma29);

         double sigma30=pow(position_cylinder[0],6)-sigma12+sigma8-sigma7-sigma29+sigma20-sigma21+sigma28+sigma9+sigma24-sigma14+sigma15-sigma22-sigma13-sigma25+sigma16-sigma17+sigma23+pow(position_wrist_home[0],6)+sigma27-sigma18+sigma19-sigma26+pow(position_cylinder[1],6)-sigma10+sigma5-sigma4+sigma6-sigma11+pow(position_wrist_home[1],6)+pow(position_cylinder[2],2)-(double)2*position_cylinder[2]*position_wrist_home[2]+pow(position_wrist_home[2],2);
         ROS_INFO("sigma30= %f",sigma30);

         double sigma31=pow(position_cylinder[0],6)-sigma12+sigma8-sigma7+sigma29-sigma20+sigma21-sigma28+sigma9-sigma24+sigma14-sigma15+sigma22-sigma13+sigma25-sigma16+sigma17-sigma23+pow(position_wrist_home[0],6)-sigma27+sigma18-sigma19+sigma26+pow(position_cylinder[1],6)-sigma10+sigma5-sigma4+sigma6-sigma11+pow(position_wrist_home[1],6)+pow(position_cylinder[2],2)-(double)2*position_cylinder[2]*position_wrist_home[2]+pow(position_wrist_home[2],2);
         ROS_INFO("sigma31 = %f",sigma30);

         double sigma3=sqrt(sigma30*sigma31);
         ROS_INFO("sigma3 = %f",sigma3);

         double sigma1=(sigma9-sigma7+sigma8-sigma6+sigma4-sigma5-2*position_cylinder[2]*position_wrist_home[2]-sigma13-sigma12+sigma11+sigma10-sigma3+pow(position_cylinder[0],6)+pow(position_wrist_home[0],6)-pow(position_cylinder[1],6)-pow(position_wrist_home[1],6)+pow(position_cylinder[2],2)+pow(position_wrist_home[2],2))/((double)2*(position_cylinder[2]-position_wrist_home[2]));
         ...
         for (int i = 0; i < int_traj; ++i)
         {
           ...
           {
            //-pi/2 -pi/2 +pi/2
             Roll_angle_traj[i]=(double)roll_wrist-(double)Roll_angle_rot_step-pi/2+pi/4;
             Pitch_angle_traj[i]=(double)pitch_wrist-(double)Pitch_angle_rot_step-pi/2;
           ...
           { 
             ...
             x_wrist_traj[i]=pow(y_wrist_traj[i]-position_cylinder[1],2)/alpha1+position_cylinder[0];
             z_wrist_traj[i]=pow(x_wrist_traj[i]-position_cylinder[0],6)/alpha2+pow(y_wrist_traj[i]-position_cylinder[1],6)/beta2+position_cylinder[2];
            }
 ...
 for (int i = 0; i <int_traj; ++i)//int_traj
  {
   ...
   waypoints.push_back(target_pose1[i]);
  }
  ...
  double eef_step((double)0.1);
  double jump_threshold((double)0.000);
  double inc_treshold((double)0.005);
  double inc_eef_step((double)0.01);
  int int_jump_max(10);

  while((double)fraction*100<(double)90)
  {  
   for (int i = 0; i < int_jump_max; ++i)
   {
     ...
     group.setPlanningTime((double)1000);
     group.setGoalOrientationTolerance((double)0); 
     group.setGoalPositionTolerance((double)0.01); 
     ...
     if((double)fraction>(double)0.90)
     {
    ...
    if((double)fraction*100>(double)90) 
    {
...
int main(int argc, char **argv) 
{
  ...
  grasp_planner::TrajectoryDemos3 node(nh);
