...
private:
 ...
 //! Subscribes to and advertises topics; initializes fitter and marker publication flags
  ...
  TrajectoryDemos(ros::NodeHandle nh) : nh_(nh), priv_nh_("~")
  {
   ...
   grasp_planning_srv_ = nh_.advertiseService(nh_.resolveName("grasp_planning_box_affordance_1_srv"),    &TrajectoryDemos::serviceCallback, this);
...
{ 
  ...
  { //sleep(4);
    ...
    tf::Vector3 position_plan(X,Y,0.10);
    ...
    box_marker.pose.orientation.x = orientation_plan[0];
    box_marker.pose.orientation.y = orientation_plan[1];
    box_marker.pose.orientation.z = orientation_plan[2];
    box_marker.pose.orientation.w = orientation_plan[3];
           
   // Set the scale of the marker -- 1x1x1 here means 1m on a side
    box_marker.scale.x = fabs(height);
    box_marker.scale.y = fabs(width);
    ...
    { 
     ...
     moveit_msgs::RobotTrajectory trajectory;
      ...
      if ((position_plane[0] != position_wrist_home[0]) & (position_plane[1] != position_wrist_home[1]))
      {
       double alpha1=-(pow(position_plane[1]-position_wrist_home[1],4))/(position_plane[0]-position_wrist_home[0]); 
       ...
       double Kp = ((double)distance_y - (double)width/2)/(double)distance_y;
       double Krot = (double)1;
       step_y=(double)distance_y/(double)int_step;
       ROS_INFO("Step_y = %f",step_y);
       
       if(position_plane[2] !=position_wrist_home[2])
       {
         // la follia non ha limiti !!!
         
         double sigma4=(double)6*pow(position_plane[1]*position_wrist_home[1],2);
          ROS_INFO("sigma4 = %f",sigma4);


         double sigma5=(double)6*pow(position_plane[0]*position_wrist_home[0],2);
         ROS_INFO("sigma5 = %f",sigma5);

         
         double sigma6=(double)4*pow(position_plane[1],3)*position_wrist_home[1];
         ROS_INFO("sigma6 = %f",sigma6);

         double sigma7=(double)4*position_plane[1]*pow(position_wrist_home[1],3);
         ROS_INFO("sigma7 = %f",sigma7);

         double sigma8=(double)4*pow(position_plane[0],3)*position_wrist_home[0];
         ROS_INFO("sigma8 = %f",sigma8);
         
         double sigma9=(double)4*position_plane[0]*pow(position_wrist_home[0],3);
         ...
         double sigma11=(double)2*pow(position_wrist_home[0]*position_plane[1],2);
         ROS_INFO("sigma11 = %f",sigma11);

         double sigma12=(double)2*pow(position_plane[0]*position_wrist_home[1],2);
         ROS_INFO("sigma12 = %f",sigma12);

         double sigma13=(double)2*pow(position_plane[0]*position_plane[1],2);
         ROS_INFO("sigma13 = %f",sigma13);
         
         double sigma14=(double)4*pow(position_wrist_home[0],2)*position_plane[1]*position_wrist_home[1];
         ROS_INFO("sigma14 = %f",sigma14);

         double sigma15=(double)4*pow(position_plane[0],2)*position_plane[1]*position_wrist_home[1];
         ROS_INFO("sigma15 = %f",sigma15);

         double sigma16=(double)4*position_plane[0]*position_wrist_home[0]*pow(position_wrist_home[1],2);
         ROS_INFO("sigma16 = %f",sigma16);

         double sigma17=(double)4*position_plane[0]*position_wrist_home[0]*pow(position_plane[1],2);
         ROS_INFO("sigma17 = %f",sigma17);

         double sigma18=(double)8*position_plane[0]*position_wrist_home[0]*position_plane[1]*position_wrist_home[1];
         ROS_INFO("sigma18 = %f",sigma18);

         double sigma19=pow(position_plane[0],4)-sigma8+sigma5-sigma13+sigma15-sigma12-sigma9+sigma17-sigma18+sigma16+pow(position_wrist_home[0],4)-sigma11+sigma14-sigma10+pow(position_plane[1],4)-sigma6+sigma4-sigma7+pow(position_wrist_home[1],4)+pow(position_plane[2],2)-(double)2*position_plane[2]*position_wrist_home[2]+pow(position_wrist_home[2],2);
         ROS_INFO("sigma19 = %f",sigma19);

         double sigma20=pow(position_plane[0],4)-sigma8+sigma5+sigma13-sigma15+sigma12-sigma9-sigma17+sigma18-sigma16+pow(position_wrist_home[0],4)+sigma11-sigma14+sigma10+pow(position_plane[1],4)-sigma6+sigma4-sigma7+pow(position_wrist_home[1],4)+pow(position_plane[2],2)-(double)2*position_plane[2]*position_wrist_home[2]+pow(position_wrist_home[2],2);
         ROS_INFO("sigma20 = %f",sigma20);

         double sigma3=sqrt(sigma19*sigma20);
         ROS_INFO("sigma3 = %f",sigma3);

         double sigma1=(sigma5-sigma4-2*position_plane[2]*position_wrist_home[2]-sigma3-sigma9-sigma8+sigma7+sigma6+pow(position_plane[0],4)+pow(position_wrist_home[0],4)-pow(position_plane[1],4)-pow(position_wrist_home[1],4)+pow(position_plane[2],2)+pow(position_wrist_home[2],2))/((double)2*(position_plane[2]-position_wrist_home[2]));
         ROS_INFO("sigma1 = %f",sigma1);

         double alpha2=position_plane[2]-position_wrist_home[2]-sigma1;
         ...
         for (int i = 0; i < int_traj; ++i)
         {
           ...
           { 
             ...
             x_wrist_traj[i]=pow(y_wrist_traj[i]-position_plane[1],4)/alpha1+position_plane[0];
             z_wrist_traj[i]=pow(x_wrist_traj[i]-position_plane[0],4)/alpha2+pow(y_wrist_traj[i]-position_plane[1],4)/beta2+position_plane[2];
            }
        ...
        for (int i = 0; i < int_traj; ++i)
        {
          ...
          if (position_wrist_home[2]>position_plane[2])
           { 
            ...
            double Kp = ((double)distance_z - (double)height/2)/(double)distance_z;
            double Krot = (double)1;
 ...
 for (int i = 0; i < int_traj; ++i)//int_traj
  ...
  {  
   for (int i = 0; i < int_jump_max; ++i)
   {
     ...
     name_file_ptr.open("/home/pacman/Projects/LUCAGEMMA/file_txt/plane_trajectory_demo.txt",std::ios::app);
...
{
  ros::init(argc, argv, "grasp_planning_box_affordance_1_node");
  ros::NodeHandle nh;
