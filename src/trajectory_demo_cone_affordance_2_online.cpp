...
private:
  ...
  bool serviceCallback(TrajectoryDemoCone::Request &request, TrajectoryDemoCone::Response &response);
 ...
 //! Subscribes to and advertises topics; initializes fitter and marker publication flags
  ...
  TrajectoryDemos(ros::NodeHandle nh) : nh_(nh), priv_nh_("~")
  {
   ...
   grasp_planning_srv_ = nh_.advertiseService(nh_.resolveName("grasp_planning_cone_affordance_2_srv"),    &TrajectoryDemos::serviceCallback, this);
 ...
 */
bool TrajectoryDemos::serviceCallback(TrajectoryDemoCone::Request &request, TrajectoryDemoCone::Response &response)
{ 
  double pi(3.1415);
...
listener_.waitForTransform("world_link","dummy_link", now0, ros::Duration(4));
  listener_.lookupTransform("world_link","dummy_link", now0, world_link_2_wrist_stamped);
  ...
  ROS_INFO ("TrajectoryDemos:Success in receving wrist transformation with 7 elements (Orient(x,y,z,w) ,Pos(x,y,z)): %f, %f ,%f, %f, %f, %f, %f",quaternion_wrist_home[0],quaternion_wrist_home[1],quaternion_wrist_home[2],quaternion_wrist_home[3],position_wrist_home[0],position_wrist_home[1],position_wrist_home[2]);
    ...
    cone = request.cone;
    tf::Transform rot_con(tf::Quaternion(cone.pose.pose.orientation.x, cone.pose.pose.orientation.y, cone.pose.pose.orientation.z, cone.pose.pose.orientation.w), tf::Vector3( cone.pose.pose.position.x,  cone.pose.pose.position.y,  cone.pose.pose.position.z));;
    
    ROS_INFO("Received for cone"); 
    ...
    con_marker.pose.orientation.x = cone.pose.pose.orientation.x;
    con_marker.pose.orientation.y = cone.pose.pose.orientation.y;
    con_marker.pose.orientation.z = cone.pose.pose.orientation.z;
    con_marker.pose.orientation.w = cone.pose.pose.orientation.w;
           
   // Set the scale of the marker -- 1x1x1 here means 1m on a side
    con_marker.scale.x = fabs(cone.h/4);
    con_marker.scale.y = fabs(cone.h/4);
    con_marker.scale.z = fabs(cone.h);

    // Set the color -- be sure to set alpha to something non-zero!
    con_marker.color.r = 0.0f;
    con_marker.color.g = 0.0f;
    con_marker.color.b = 1.0f;
    ...
    { 
     ...
     moveit_msgs::RobotTrajectory trajectory;
      ...
      if ((position_cone[0] != position_wrist_home[0]) & ( position_wrist_home[2] != (double)0))
      {
       double alpha=-(pow(position_wrist_home[2],6))/(position_cone[0]-position_wrist_home[0]); 
       double distance_z=position_wrist_home[2]-position_cone[2];
       double Kp = ((double)distance_z - (double)cone.h/2 -(double)threshold_distance/(double)4)/(double)distance_z;
       double Krot = (double)1;
       ROS_INFO("Alpha = %f",alpha);
       ROS_INFO("Calculated distance_y: %f",distance_z);
       step_z=(double)distance_z/(double)int_step;
       ROS_INFO("Step_z = %f",step_z);
       
        if((position_cone[1] !=position_wrist_home[2])&(position_wrist_home[2]!=(double)0))
        {
        double beta=-(pow(position_wrist_home[2],4))/(position_cone[1]-position_wrist_home[1]);    
        ROS_INFO("Beta:%f ",beta);

        ROS_INFO("Start trajectory for generic position of the cone");
        ros::Time now = ros::Time::now();
        ...
        if(abs(distance_x) > abs(distance_y))
         ...
         for (int i = 0; i < int_traj; ++i)
         {
           ...
           { 
             ROS_INFO("Wrist is on cone but in different x and y");
             if(i==0) 
             {
             ...
             x_wrist_traj[i]=pow(z_wrist_traj[i],6)/alpha+position_cone[0];
             y_wrist_traj[i]=pow(z_wrist_traj[i],4)/beta+position_cone[1];
            }
          
            else
            { 
             
             ROS_INFO("Wrist is under the cone but in different x and y");
             if(i==0) 
             {
               z_wrist_traj[i]=position_wrist_home[2]+Kp*step_z;
              }
       ...
       { 
         ...
         ROS_INFO("Wrist is on the same z of the cone");
         ros::Time now = ros::Time::now();
         //tf::Quaternion quaternion_wrist_final(tf::createQuaternionFromRPY(0, 0, -pi/2));
         double Roll_wrist_end,Pitch_wrist_end,Yaw_wrist_end;
         ...
         // same z but x and y different for wrist and cone
         ...
         for (int i = 0; i < int_traj; ++i)
         {
           ...
           { 
             ROS_INFO("Wrist is on the right of cone");
             double distance_y=position_cone[1]-position_wrist_home[1];
             ...
             x_wrist_traj[i]=pow(y_wrist_traj[i]-position_cone[1],4)/alpha+position_cone[0];
            }
          
            else
            { 
             ROS_INFO("Wrist is on the left of the cone");
             double distance_y=position_cone[1]-position_wrist_home[1];
             ROS_INFO("Calculated distance_y: %f",distance_y);
             double Kp = ((double)distance_y - (double)cone.h/2 -(double)threshold_distance)/(double)distance_y;
             double Krot = (double)1;
             double step_y=distance_y/int_step;
             if(i==0) 
             {
      ...
      { 
       ...
       // same x and y for wrist and cone !!!
       ROS_INFO("Wrist is on or down the cone");
       ros::Time now = ros::Time::now();
        //tf::Quaternion quaternion_wrist_final(tf::createQuaternionFromRPY(0, 0, -pi/2));
        double Roll_wrist_end,Pitch_wrist_end,Yaw_wrist_end;
        ...
        for (int i = 0; i < int_traj; ++i)
        {
          ...
          { 
            ROS_INFO("Wrist is down the cone");
 ...
 for (int i = 0; i < int_traj; ++i)//int_traj
  ...
  {  
   for (int i = 0; i < int_jump_max; ++i)
   {
     ...
     name_file_ptr.open("/home/pacman/Projects/LUCAGEMMA/file_txt/cone_affordance_2_trajectory_demo.txt",std::ios::app);
     ...
     name_file_ptr << "\n";
     name_file_ptr.close();  
     ROS_INFO("Data wrote into the file cone_affordance_2_trajectory_demo.txt");
...
{
  ros::init(argc, argv, "grasp_planning_cone_affordance_2_node");
  ros::NodeHandle nh;
