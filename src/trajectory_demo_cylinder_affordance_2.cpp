...
{ 
  ...
  { //sleep(4);
    ...
    cyl_marker.ns = "trajectory_demo_sphere_node";
    ...
    { 
     ...
     moveit_msgs::RobotTrajectory trajectory;
      ...
      if ((position_cylinder[0] != position_wrist_home[0]) & ( position_wrist_home[2] != (double)0))
      {
       ...
       double Kp = ((double)distance_z - (double)cylinder.h/2)/(double)distance_z;
       ...
       ROS_INFO("Step_z = %f",step_z);
        ...
        if(abs(distance_x) > abs(distance_y))
         ...
         for (int i = 0; i < int_traj; ++i)
         {
           ...
           { 
             ROS_INFO("Wrist is on cylinder but in different x and y");
             if(i==0) 
             {
 ...
 for (int i = 0; i < int_traj; ++i)//int_traj
  ...
  {  
   for (int i = 0; i < int_jump_max; ++i)
   {
     ...
     name_file_ptr.open("/home/pacman/Projects/LUCAGEMMAfile_txt/cylinder_affordance_2_trajectory_demo.txt",std::ios::app);
