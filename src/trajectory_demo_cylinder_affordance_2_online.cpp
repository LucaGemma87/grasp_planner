...
private:
 ...
 //! Subscribes to and advertises topics; initializes fitter and marker publication flags
  ...
  TrajectoryDemos(ros::NodeHandle nh) : nh_(nh), priv_nh_("~")
  {
   ...
   grasp_planning_srv_ = nh_.advertiseService(nh_.resolveName("grasp_planning_cylinder_affordance_2_srv"),    &TrajectoryDemos::serviceCallback, this);
  ...
  ROS_INFO ("TrajectoryDemos:Success in receving wrist transformation with 7 elements (Orient(x,y,z,w) ,Pos(x,y,z)): %f, %f ,%f, %f, %f, %f, %f",quaternion_wrist_home[0],quaternion_wrist_home[1],quaternion_wrist_home[2],quaternion_wrist_home[3],position_wrist_home[0],position_wrist_home[1],position_wrist_home[2]);
   ...
   // Set the scale of the marker -- 1x1x1 here means 1m on a side
    ...
    { 
       ...
       //test for orientataion
      tf::Vector3 u = rotation_cylinder.getColumn(0);
      tf::Vector3 v = rotation_cylinder.getColumn(1);
      tf::Vector3 w = rotation_cylinder.getColumn(2);
      tf::Transform identity_transform(tf::Quaternion(tf::createQuaternionFromRPY(0,0,0)), tf::Vector3( 0,0,0));
      tf::Matrix3x3 identity_rotation = identity_transform.getBasis();
      tf::Vector3 x = identity_rotation.getColumn(0);
      tf::Vector3 y = identity_rotation.getColumn(1);
      tf::Vector3 z = identity_rotation.getColumn(2);
      bool up_side=false;
      if (w.dot(z) > 0.7 || w.dot(z) < -0.7) up_side = true;

      //Define RPY of the cylinder
     ...
     moveit_msgs::RobotTrajectory trajectory;
      ...
      if ((position_cylinder[0] != position_wrist_home[0]) & ( position_wrist_home[2] != (double)0))
      {
       double alpha=-(pow(position_wrist_home[2],6))/(position_cylinder[0]-position_wrist_home[0]); 
       double distance_z=position_wrist_home[2]-position_cylinder[2];
       double Kp;
       if(up_side) 
       {
        Kp = ((double)distance_z - (double)cylinder.h/2 -(double)threshold_distance)/(double)distance_z;
       }
       else  
        { 
          threshold_distance=(double)0.01;
          Kp = ((double)distance_z - (double)cylinder.r -(double)threshold_distance)/(double)distance_z;
        }
       ...
       ROS_INFO("Step_z = %f",step_z);
        ...
        if((position_cylinder[1] !=position_wrist_home[2])&(position_wrist_home[2]!=(double)0))
        {
        //double beta=-(pow(position_wrist_home[2],4))/(position_cylinder[1]-position_wrist_home[1]);   
         double beta=-(pow(position_wrist_home[2],6))/(position_cylinder[1]-position_wrist_home[1]);    
        ...
        if(abs(distance_x) > abs(distance_y))
           ...
           if (up_side)
          {   
          ROS_INFO("The cylinder is considered to be up side");  
          Roll_wrist_end=(double)(roll_wrist-pi/2);//-theta
         Pitch_wrist_end=(double)(pitch_wrist-pi/2);
         Yaw_wrist_end=(double)(yaw_wrist-pi/2);
         ...
         {
          ROS_INFO("The cylinder is considered to be lying down");
         Roll_wrist_end=(double)(yaw_cyl+pi/2);//-theta
         //buona la riga successiva   
         ...
         for (int i = 0; i < int_traj; ++i)
         {
           ...
           { 
             ROS_INFO("Wrist z  is on cylinder but in different x and y");
             if(i==0) 
             {
             ...
             x_wrist_traj[i]=pow(z_wrist_traj[i],6)/alpha+position_cylinder[0];
             //y_wrist_traj[i]=pow(z_wrist_traj[i],4)/beta+position_cylinder[1];
             y_wrist_traj[i]=pow(z_wrist_traj[i],6)/beta+position_cylinder[1];
            }
          
            else
            { 
             
             ROS_INFO("Wrist is under the cylinder but in different x and y");
             if(i==0) 
             {
               z_wrist_traj[i]=position_wrist_home[2]+Kp*step_z;
              }
       ...
       { 
         ...
         //tf::Quaternion quaternion_wrist_final(tf::createQuaternionFromRPY(0, 0, -pi/2));
         double Roll_wrist_end,Pitch_wrist_end,Yaw_wrist_end;
         Roll_wrist_end=(double)(-pi/2+theta);//-theta
         //Riga buona successiva
         //Roll_wrist_end=(double)(-pi/2-theta);
         Pitch_wrist_end=(double)0;
         Yaw_wrist_end=(double)0;
         ...
         for (int i = 0; i < int_traj; ++i)
         {
           ...
           { 
             ...
             x_wrist_traj[i]=pow(y_wrist_traj[i]-position_cylinder[1],4)/alpha+position_cylinder[0];
            }
 ...
 for (int i = 0; i < int_traj; ++i)//int_traj
  ...
  {  
   for (int i = 0; i < int_jump_max; ++i)
   {
     ...
     // name_file_ptr << "\n";
     // name_file_ptr.close();  
     // ROS_INFO("Data wrote into the file cylinder_affordance_2_trajectory_demo.txt");
    ...
    response.moveit_trajectory= trajectory;    response.moveit_trajectory= trajectory;
...
{
  ros::init(argc, argv, "grasp_planning_cylinder_affordance_2_node");
  ros::NodeHandle nh;
