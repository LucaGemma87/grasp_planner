...
private:
 ...
 //! Subscribes to and advertises topics; initializes fitter and marker publication flags
  ...
  TrajectoryDemos(ros::NodeHandle nh) : nh_(nh), priv_nh_("~")
  {
   ...
   grasp_planning_srv_ = nh_.advertiseService(nh_.resolveName("grasp_planning_sphere_affordance_2_srv"),    &TrajectoryDemos::serviceCallback, this);
...
{ 
  ...
  double pi(3.1415);
  double threshold_distance((double)0.005);
  ...
  ROS_INFO ("TrajectoryDemos:Success in receving wrist transformation with 7 elements (Orient(x,y,z,w) ,Pos(x,y,z)): %f, %f ,%f, %f, %f, %f, %f",quaternion_wrist_home[0],quaternion_wrist_home[1],quaternion_wrist_home[2],quaternion_wrist_home[3],position_wrist_home[0],position_wrist_home[1],position_wrist_home[2]);
    ...
    // Set the marker type to sphINDER
   ...
   // Set the scale of the marker -- 1x1x1 here means 1m on a side
    ...
    { 
      ...
      // rotation_sphere.getRPY(roll_cyl, pitch_cyl, yaw_cyl);
     ...
     moveit_msgs::RobotTrajectory trajectory;
      ...
      if ((position_sphere[0] != position_wrist_home[0]) & ( position_wrist_home[2] != (double)0))
      {
       double alpha=-(pow(position_wrist_home[2],6))/(position_sphere[0]-position_wrist_home[0]); 
       ...
       ROS_INFO("Step_z = %f",step_z);
        ...
        if((position_sphere[1] !=position_wrist_home[2])&(position_wrist_home[2]!=(double)0))
        {
        double beta=-(pow(position_wrist_home[2],4))/(position_sphere[1]-position_wrist_home[1]);    
        ...
        Roll_wrist_end=(double)(roll_wrist-pi/2);//-theta
         ...
         for (int i = 0; i < int_traj; ++i)
         {
           ...
           { 
             ROS_INFO("Wrist is on sphere but in different x and y");
             if(i==0) 
             {
             ...
             x_wrist_traj[i]=pow(z_wrist_traj[i],6)/alpha+position_sphere[0];
             y_wrist_traj[i]=pow(z_wrist_traj[i],4)/beta+position_sphere[1];
            }
          
            else
            { 
             
             ROS_INFO("Wrist is under the sphere but in different x and y");
             if(i==0) 
             {
               z_wrist_traj[i]=position_wrist_home[2]+Kp*step_z;
              }
       ...
       { 
         ...
         for (int i = 0; i < int_traj; ++i)
         {
           ...
           { 
             ...
             x_wrist_traj[i]=pow(y_wrist_traj[i]-position_sphere[1],4)/alpha+position_sphere[0];
            }
          ...
          if (position_wrist_home[2]>position_sphere[2])
           { 
            ...
            ROS_INFO("Calculated distance_z: %f",distance_z);
             double Kp = ((double)distance_z - (double)sphere.r- (double)threshold_distance)/(double)distance_z;
            double Krot = (double)1;
 ...
 for (int i = 0; i < int_traj; ++i)//int_traj
  ...
  {  
   for (int i = 0; i < int_jump_max; ++i)
   {
     ...
     // name_file_ptr.open("/home/pacman/Projects/LUCAGEMMA/file_txt/sphere_affordance_2_trajectory_demo.txt",std::ios::app);
     ...
     // name_file_ptr << "\n";
     // name_file_ptr.close();  
     // ROS_INFO("Data wrote into the file sphere_affordance_2_trajectory_demo.txt");
...
{
  ros::init(argc, argv, "grasp_planning_sphere_affordance_2_node");
  ros::NodeHandle nh;
